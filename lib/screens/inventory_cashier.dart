...
// Wrapper to expose this screen as the cashier Inventory tab without altering existing code
class InventoryCashier extends StatelessWidget {
  const InventoryCashier({super.key});

  @override
  Widget build(BuildContext context) {
    return const ProductionTab();
  }
}
...
class _ProductionTabState extends State<ProductionTab>
  ...
  late Timer _stockPollTimer;
  bool _isFetching = false;
  // Live discrepancies state (Cashier Inventory tab)
  ...
  // Mock, live-updating inventory data (prev/current to compute delta)
  ...
  void initState() {
    ...
    // Ensure Ice Cube (row 1) animates numbers on first load like Ice Block
    if (_inventoryData.length > 1) {
      _inventoryData[1]['prevInStock'] = 0;
      _inventoryData[1]['prevInProduction'] = 0;
    }

    // Periodically update only In Production mock values; In Stock comes from backend
    _inventoryTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      setState(() {
        for (int i = 0; i < _inventoryData.length; i++) {
          final row = _inventoryData[i];
          // In Production
          if (i == 0) {
            // Ice Block: keep in-production equal to live Production Today
            row['prevInProduction'] = _prevProductionToday;
            row['inProduction'] = _productionToday;
            _scheduleDeltaHide(
              row,
              'prevInProduction',
              'inProduction',
            ...
            // Increase-only mock for smoother growth
            final prodDelta = _rng.nextInt(4) + 1; // 1..4
            row['prevInProduction'] = currProd;
            row['inProduction'] = (currProd + prodDelta).clamp(0, 9999);
            _scheduleDeltaHide(
              row,
              'prevInProduction',
              'inProduction',
    ...
    // Initialize repository with Authorization header and poll backend for In Stock
    Future<void> _setupInventoryCashier() async {
      String? token;
      try {
      ...
      await _fetchAndApplyInventoryCashier();
    }

    _setupInventoryCashier();
    _stockPollTimer = Timer.periodic(const Duration(milliseconds: 150), (_) {
      _fetchAndApplyInventoryCashier();
    });
    // Setup live discrepancies (SSE + fast polling)
  ...
  Future<void> _fetchAndApplyInventoryCashier() async {
                ...
                Builder(
                  builder: (context) {
                    ...
                    return Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        // Fixed header (looks the same as before)
                        DataTable(
